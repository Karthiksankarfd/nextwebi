...
const Navbar = () => {
    ...
    useEffect(() => {
        const handleScroll = () => {
          ...
          if (!nav) return; // if element not found
  ...
  return (
    ...
    <nav className='w-[100%] transition-all duration-400  font-[var(--fw-semibold)]'> 
        <section id='navbar-inner-section'>
             <div  className='  transition-all  max-w-[1380px] w-[100%] p-4 md:p-4  mx-auto flex items-center   justify-between text-[var(--text-btn-color)] '>
              ...
              <div className="call-to-action-btns  flex gap-x-4 w-fit">
                  ...
                  <button className='p-2 block md:hidden' onClick={()=>setSliderMenu((prev)=> !prev)}><FaList/></button>
              </div>
          </div>
        </section>
        ...
        {isSliderMenu && 
            <nav onClick={()=>setSliderMenu((prev)=> !prev)} className='h-dvh w-full inset-0  bg-black/30 absolute z-[5] overflow-x-hidden'>  
